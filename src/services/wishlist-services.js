...
const WishlistService = {
  ...
  getWishlistedGamesByUser(db, user_id) {
    return db.select("vgfinder_games.*").from(wishlistsTable)
      .join("vgfinder_games", "vgfinder_wishlists.game_id", "=", "vgfinder_games.id").where({ user_id });
  },
  addGameToUserWishlist(db, wishlistObj) {
    return db.select("*").insert(wishlistObj).into(wishlistsTable).returning("*").then(rows => {
      return rows[0];
    });
  },
