...
gamesRouter.route("/")
  ...
  .post(jwtAuth, jsonBodyParser, (req, res, next) => {
    ...
    //Note: taking this out for now, but might be cool to have a contributor field
...
gamesRouter.route("/:game_id")
  ...
  //public endpoint
  .get((req, res) => {
    GamesService.getGameById(req.app.get("db"), req.params.game_id)
  ...
  .delete(jwtAuth, (req, res, next) => {
    GamesService.removeGameById(req.app.get("db"), req.params.game_id)
      .then(game => res.status(204).json(GamesService.serializeGame(game)).end()) //doesn't actually return anything
      .catch(next);
  });
...
async function checkGameExists(req, res, next) {
  try {
    const game = await GamesService.getGameById(req.app.get('db'), req.params.game_id)
    if (!game)
